...
export const fetchFollowStatus = async (currentUserId, targetUserId) => {
    const response = await fetch(`${API_URL}/users/${targetUserId}/relationships?current_user_id=${currentUserId}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Failed to fetch relationship status");
    return data;
};
...
export const createTransaction = async (transactionPayload) => {
    ...
    const data = await response.json().catch(() => {
        throw new Error("Server returned an invalid JSON response or status 500/400.");
    });
...
export const fetchTransactionDetails = async (transactionId) => {
    ...
    if (!response.ok) {
        const errorBody = await response.json().catch(() => ({error: `Failed to fetch receipt ${transactionId}`}));
...
export const reportTransactionIssue = async (transactionId, reason) => {
    const response = await fetch(`${API_URL}/transactions/${transactionId}/report`, {
        method: "PUT",
        headers: getAuthHeaders(),
        body: JSON.stringify({ reason })
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Failed to report transaction");
    return data;
};
...
export const removeFromCart = async (productId, userId) => {
    const response = await fetch(`${API_URL}/cart/${productId}?user_id=${userId}`, {
        method: "DELETE",
    });
...
export const updateRentalStatus = async (requestId, status, currentUserId, reason = null) => {
    const response = await fetch(
    ...
    const data = await response.json(); 
    if (!response.ok) throw new Error(data.error || "Failed to update rental status");
...
export const createSwapRequest = async (swapData) => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Failed to create swap request");
    return data;
};
...
export const updateSwapStatus = async (swapId, status, currentUserId, reason = null) => {
...
export const fetchThreadMessages = async (otherUserId, currentUserId) => {
    const response = await fetch(`${API_URL}/messages/thread/${otherUserId}?current_user_id=${currentUserId}`); 
...
export const markNotificationsRead = async (ids = null, markAll = false, userId) => {
    try {
        const payload = {
            user_id: userId,
            mark_all: markAll,
...
export const fetchUserAverageRating = async (userId) => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Failed to fetch average rating");
    return data;
};
...
export const fetchAllUserReviews = async (userId) => {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Failed to fetch user reviews");
    return data;
};
